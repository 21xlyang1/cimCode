...
api_info_url = "https://vcp.21cn.com/open/token/device/showDevice"
...
# accessToken有效时长24小时
accessToken = "HagmCabPTzj2JBhIKHHdghE3PydRSxKW"
...
def getRtsp(deviceName, deviceCode):
    paramRtsp = "deviceCode=%s&accessToken=%s" % (deviceCode, accessToken)
    encryptParam = xxtea.encrypt(paramRtsp, appSecret).hex()
    realRtspParam = public_params(encryptParam)

    repsonse = send_post(api_info_url, realRtspParam)
